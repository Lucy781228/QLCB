<template>
    <form method="post">
        <NcModal v-if="modal" @close="closeModal" title="THÊM QUÁ TRÌNH KHEN THƯỞNG">
            <div class="modal__content">
                <div class="block-list">
                    <div class="block-list__item">
                        <label>Số quyết định</label>
                        <NcTextField :value.sync="khenThuong.soQuyetDinh" />
                        <span class="error" :class="{ active: numberError }">{{ numberError }}</span>
                    </div>
                    <div class="block-list__item">
                        <label>Cơ quan quyết định</label>
                        <NcTextField :value.sync="khenThuong.coQuanQuyetDinh" />
                        <span class="error" :class="{ active: departmentError }">{{ departmentError }}</span>
                    </div>
                    <div class="block-list__item">
                        <label>Thời gian</label>
                        <NcTextField :value.sync="khenThuong.thoiGian" type="date" />
                        <span class="error" :class="{ active: timeError }">{{ timeError }}</span>
                    </div>
                    <div class="block-list__item">
                        <label>Hình thức</label>
                        <NcTextField :value.sync="khenThuong.hinhThuc" />
                        <span class="error" :class="{ active: formError }">{{ formError }}</span>
                    </div>
                    <div class="block-list__item">
                        <label>Lý do</label>
                        <NcTextField :value.sync="khenThuong.lyDo" />
                    </div>
                </div>
                <br>
        ...
        async createUser(event) {
                ...
                try {
                    const response = await axios.post('/apps/kmaapp/create_kma_bonus', {
                        ...
                        time: this.khenThuong.thoiGian,
                        form: this.khenThuong.hinhThuc,
                        number_decision: this.khenThuong.soQuyetDinh,
                        department_decision: this.khenThuong.coQuanQuyetDinh,
                        reason: this.khenThuong.lyDo,
                        type: 1
                    });
                    this.$emit('refresh-khenthuong-list');
